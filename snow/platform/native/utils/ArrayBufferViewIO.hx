...
class ArrayBufferViewIO {
    ...
    @:generic
    public static function setFromArray<T_from_arr>( view:ArrayBufferView, array:Array<T_from_arr>, ?offset : Int = 0 ) {

        switch(view.type) {
            case Float32: setFromFloatArray(view, cast array, offset);
            case _: setFromIntArray(view, cast array, offset);
        }

        // #if cpp

                //:todo: the .blit calls are using elements counts, where getData()
                //will always return 1 because its bytes underneath, need a blit with byte values
                //so that it can be copied directly from the array :<
            //cpp.NativeArray.blit( view.buffer.getData(), offset, cast array, 0, array.length);
...
//Internal
    ...
    inline static function setFromFloatArray( view:ArrayBufferView, array:Array<Float>, ?offset : Int = 0 ) {
    ...
    inline static function setFromIntArray( view:ArrayBufferView, array:Array<Int>, ?offset : Int = 0 ) {
